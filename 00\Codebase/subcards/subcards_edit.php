<?php
// subcards/edit.php - ویرایش زیرکارت
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        // Update subcard
        $stmt = $pdo->prepare("UPDATE subcards SET title = ?, description = ? WHERE id = ?");
        $stmt->execute([$title, $description, $subcard_id]);
        
        echo json_encode(['success' => true]);
        
    } catch (Exception $e) {
        echo json_encode(['success' => false, 'error' => 'خطا در ویرایش زیرکارت: ' . $e->getMessage()]);
    }
} else {
